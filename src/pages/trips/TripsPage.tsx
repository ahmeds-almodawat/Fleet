...
export default function TripsPage() {
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter((trip) => {
      ...
      const matchesDept =
        ...
        (trip.vehicle?.department_id && trip.vehicle.department_id === departmentFilter);

      // If user cannot view all, keep only own/related (DB RLS already enforces too)
  ...
  const statusBadge = (status: string) => {
    ...
    switch (status) {
      ...
      case 'Completed':
        return <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100">{t(key)}</Badge>;
      case 'Rejected':
      case 'Cancelled':
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  {['Draft', 'PendingApproval', 'Approved', 'InProgress', 'Completed', 'Rejected', 'Cancelled'].map((s) => (
                    <SelectItem key={s} value={s}>
                      {t(`trips.status.${s}`, { defaultValue: s })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                {filteredTrips.map((trip) => (
                  <tr key={trip.id} className="border-t">
                    <td className="px-4 py-3">
                      <Link to={`/trips/${trip.id}`} className="font-medium text-primary hover:underline">
                        {trip.trip_no}
                      </Link>
                    </td>
                      ...
                      <div className="space-y-1">
                        ...
                        {trip.job_order_no && (
                          <div className="text-xs text-muted-foreground">
                            {t('trips.new.label.jobOrderNo')}: {trip.job_order_no}
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className={cn('px-4 py-3 text-sm tabular-nums', isRtl ? 'text-left' : 'text-right')}>
                      {trip.distance_km ? (
                        <span>
                          {Math.round(trip.distance_km * 10) / 10}
                          <span className={cn('text-muted-foreground', isRtl ? 'mr-1' : 'ml-1')}>{t('trips.km')}</span>
                        </span>
                      ) : '—'}
                    </td>
